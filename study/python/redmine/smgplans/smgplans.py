...
def fillPlans():
    for user in users:
        ...
        issues = redmine.issue.filter(project_id='voip-smg', assigned_to_id=user["uid"], cf_1="~"+month_tag)
        if len(issues):
            for issue in issues:
                task_list = list()
...
def writeReport():
    cvsf = open("report/"+month_tag+"_"+week_tag+".csv",'w')
    cvsf.write("User;Project;Version;Ref;Status;Subject;Tags;Hours;Priority\n")
    ...
    for user in users:
        printTrable = PrettyTable(['Project', 'Version', 'Ref', 'Status', 'Prio', 'Time', 'Subject'])
        ...
        if task_count == 0:
            ...
            for task in user["plan"]:
                ...
                printTrable.add_row([task[0], task[1], task[2], task[3], task[7], "{:3.2f}".format(task[6]), task[4]])
    ...
    print("Распределение задач по версиям на " + month_tag + " " + week_tag)
...
def sendMailNotify(login, passwd):
    try: 
        ...
        msg['Subject'] = "[" + month_tag + " " + week_tag + "] Планы на неделю"
        html = """\
        <html>
          <p style="color:gray;">Сообщение сгенерировано автоматически</p>
          <h1 style="color: red;">Не проставлены теги планов на неделю</h1>
          <body>
            <p>Вы получили это сообщение потому, что ни в одной задаче не проставлены теги планов на неделю<br>
               Теги на след.неделю: """
        html += month_tag + " " + week_tag + "<br></p>"
...
for idx, arg in enumerate(sys.argv):
    if arg == "--help":
        ...
        sys.exit()
    if arg == "--month":
        month_tag = "plan_month_%02d" % int(sys.argv[idx+1])
    if arg == "--week":
        week_tag = "plan_week_%02d" % int(sys.argv[idx+1])
    if arg == "--mail":
        mail_notify = "yes"

if len(month_tag) == 0:
    month_tag = "plan_month_%02d" % datetime.now().month

if len(week_tag) == 0:
    week_tag = "plan_week_%02d" % (datetime.now().isocalendar()[1] + 1)
